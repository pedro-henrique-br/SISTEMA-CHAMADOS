...
const TicketAssignedTable = ({ priorityColors, tecnicos }) => {
    const { ticketsAssignedArray, fetchTickets, deleteTicket, addObservation, updateTicket, initPusher } = useTicketStore();
    ...
    useEffect(() => {
        const fetchAvatars = async () => {
            const avatarPromises = ticketsAssignedArray?.map(async (ticket) => {
    ...
    const filteredTickets = Array.isArray(ticketsAssignedArray) ? ticketsAssignedArray
        .filter(ticket => {
    ...
    return (
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }}>
            <Typography variant="h5" gutterBottom>Chamados Em Atendimento</Typography>
            ...
            {ticketsAssignedArray?.length > 0 ? (
                                        ...
                                        <TableCell sx={{ padding: "6px" }}>
                                            <Select
                                                ...
                                                renderValue={(selected) =>
                                                    <Box sx={{ display: "flex", alignItems: "center", gap: "4px" }}>
                                                        <Avatar src={avatars[selected || ticket.tecnico]} sx={{ width: 24, height: 24 }} />
                                                        <Box sx={{ display: "flex", flexDirection: "column" }}>
                                                            <Typography sx={{ textTransform: "capitalize", fontSize: "0.75rem" }} variant="body2">
                                                                {ticket.tecnico}
                                                            </Typography>
                                        ...
                                        <TableCell sx={{ padding: "6px" }}>
                                            ...
                                            >
                                                {Object.keys(priorityColors).map((prioridade) => (
                                                    <MenuItem key={prioridade} value={prioridade} sx={{ fontSize: "0.7rem" }}>
                                                        {prioridade}
                                                    </MenuItem>
                                                ))}
                                            </Select>
                ...
                : (
                    <Box sx={{ textAlign: "center", mt: 4 }}>
                        <Typography variant="h6" color="primary">🎉 Nenhum Chamado Sendo atendido por outro Tecnico!</Typography>
...
export default TicketAssignedTable;
